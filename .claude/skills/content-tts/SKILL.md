...
| `Editor/Toolbar/ToolbarTts.svelte`             | **The read-aloud bar** — Skip back, Stop, Play, Skip forward and the voice popover on one floating surface, and the one `Tooltip.Provider` behind them. `onClose` makes Stop also put the bar away (live even when idle); `autofocus` puts focus on Play as it opens. Takes a `controller` prop so a story can drive all five from a chosen state.                      |
| `Editor/Toolbar/ToolbarStop.svelte`            | Stop button (`speech.stop()`); leaves the page where it is, deliberately. With `onStop` it also closes the bar, and stays enabled when nothing plays.                                                                                                                                                                                                                   |
| `Editor/Toolbar/ToolbarReadAloud.svelte`       | The toolbar's one read-aloud control: a `Toggle` that opens the bar and is pressed while it is open. Opening starts no read — the writer presses Play in the bar. Pressing it again closes the bar and stops any read.                                                                                                                                                  |
| `$lib/tts/read-aloud-bar.svelte.ts`            | `ReadAloudBar` — whether the bar is open: `launched \|\| isPlaying`, with `launched` cleared the moment a read starts. So every way a read ends (Stop, the end of the text, each `speech.stop()` the page makes) also closes the bar, and a pause keeps it. Unit-tested.                                                                                                |
...
`Editor/Page/PageEditor.svelte`; the toolbar row ends in `Toolbar.ReadAloud`, and the
transport (`Toolbar.Tts`) floats in the canvas's top-right corner through
`Page`'s `controls` snippet while `ReadAloudBar` holds it open — both in
`routes/edit/+page.svelte`, which also hydrates `speech`
