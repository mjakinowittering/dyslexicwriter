...
- **`### Bugs`** — something **already built** that doesn't behave as intended.
  Whether a writer would notice it or only a developer makes no difference: an
  invisible table and a muted test suite are both bugs.
...
Inside each list, related items sit under a `####` **theme heading** — "Images and
tables in the editor", "Deployment", "Welcome and first run". The themes are part of
...
indented six spaces, and some carry sub-bullets at four — all of that belongs to the
parent item, not to a new one. (Prettier leaves README prose alone, so the wrapping
is yours to keep.) Number items `1..N` **straight through both lists** — Bugs first,
...
pair — fixing the sentence split and adding auto-scroll both live in read-aloud.
...
- what the user will see afterwards — as ASCII wireframes for any visible change,
  ...
  changes in each
- the CLAUDE.md rules it brushes against, new Paraglide keys, and the tests: a
  round-trip case in the same commit as any new editor node, an OPFS case before
  anything in `lib/fs/` changes, and the autosave lifecycle where the document store
  moves (`testing`)
...
built that misbehaves is a Bug however small its audience — the invisible-table item is
filed there because tables insert fine and simply can't be seen. Something that has
...
- cites files and symbols in backticks (`fs/documents.ts:885`, `FormatInsertImage.svelte`)
- names the fix when known, and fences the scope if it could read bigger than it is
  ("column resizing is off by design")
