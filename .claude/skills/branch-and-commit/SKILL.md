...
- **Prefix** — `bug/` for something already built that doesn't behave as intended,
  whether a writer or only a developer would notice; `feature/` for work not yet
...
```
bug/list-markers-sit-below-their-line
feature/editor-invisible-characters
```
...
- **Hand the message over** — the user commits unless they ask you to. If you commit,
  end with the `Co-Authored-By` trailer. There is no pre-commit hook: `npm run lint`
  is the only thing that checks formatting, and it is on you to have run it.

The message: a short imperative subject, a blank line, a bulleted body.

```
Stop list markers sitting a line-margin below their text

- Drops the marker's own margin-top from bullets and numbers: the
  li p margin collapses out through the <li>, so the item's top is
  already the first line's top and the offset counted it twice
- Leaves the checkbox's offset in place — a task item is a flex
  container, which keeps the margin inside, so there it is needed
- Numbers now share the text's baseline and bullets sit mid-line,
  at every prose size rather than one tuned by a fixed offset
```
